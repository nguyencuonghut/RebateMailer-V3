                                    ...
                                    >
                                        <div class="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
                                            ...
                                            <div v-if="canManageTemplates && isFilteredView" class="flex flex-wrap gap-2 xl:flex-nowrap xl:justify-end">
                                                ...
                                                <template v-else-if="element.type === camCaSectionType">
                                                    <Button label="Dữ liệu" size="small" variant="outlined" @click="addCamCaRow(element.type, 'value-row')" />
                                                    <Button label="Mục cha" size="small" variant="outlined" @click="addCamCaRow(element.type, 'parent')" />
                                                    <Button label="Mục con" size="small" variant="outlined" @click="addCamCaRow(element.type, 'child-value')" />
                                                    <Button label="Dòng CT" size="small" variant="outlined" @click="addCamCaRow(element.type, 'child-program-loop')" />
                                        ...
                                        >
                                            <template #item="{ element: row, index: rowIndex }">
                                                ...
                                                >
                                                    ...
                                                    <div v-else-if="element.type === camCaSectionType" class="space-y-3">
                                                        <div class="space-y-3">
                                                            <div class="flex flex-col gap-3 xl:flex-row xl:items-start">
                                                                ...
                                                                >
                                                                    ...
                                                                    <div class="min-w-0 xl:w-[24rem]">
                                                                        <Select
                                                                            :model-value="row.columnKey ?? null"
                                                                            :options="camCaBindingOptions"
                                                                            option-label="label"
                                                                            option-value="key"
                                                                            filter
                                                                            show-clear
                                                                            fluid
                                                                            :disabled="!isFilteredView || !canManageTemplates"
                                                                            placeholder="Chọn key dữ liệu Cám cá"
                                                                            @update:model-value="updateCamCaColumnKey(element.type, row.renderKey, $event)"
                                                                        />
                                                                    </div>
                                                                </div>
                                                                ...
                                                                >
                                                                    <div class="min-w-0 flex-1">
                                                                        <template v-if="row.rowType === 'child-program-loop'">
                                                                            <div
                                                                                ...
                                                                                :style="{ borderColor: 'var(--dashboard-panel-border)', color: 'var(--dashboard-muted-text)' }"
                                                                            >
                                                                                Lặp qua các CT có dữ liệu thật từ <code>programItems[]</code>. Preview và email sẽ ẩn CT nếu cả `Nội dung | Thành tiền` đều bằng `0` hoặc rỗng.
                                                                            </div>
                                                                        </template>
                                                                        <template v-else-if="row.rowType === 'blank'">
                                                                            <div
                                                                                ...
                                                                                :style="{ borderColor: 'var(--dashboard-panel-border)', color: 'var(--dashboard-muted-text)' }"
                                                                            >
                                                                                Dòng trống để tạo khoảng cách giữa các block của bảng Cám cá.
                                                                            </div>
                                                                        </template>
                                                                        <template v-else>
                                                                            <InputText
                                                                    ...
                                                                    <div v-if="canManageTemplates && isFilteredView" class="flex flex-wrap items-center gap-2 xl:justify-end">
                                                                        <Button
                                                                            ...
                                                                            :severity="row.isBold ? 'info' : 'secondary'"
                                                                            variant="outlined"
                                                                            @click="toggleCamCaRowBold(element.type, row.renderKey)"
                                                                        />
                                                                        <Button
                                                                            v-if="['value-row', 'child-value'].includes(row.rowType ?? '')"
                                                                            :label="row.hideWhenValueZero ? 'Ẩn khi = 0 hoặc rỗng' : 'Hiện cả = 0 hoặc rỗng'"
                                                                            ...
                                                                            :severity="row.hideWhenValueZero ? 'warn' : 'secondary'"
                                                                            variant="outlined"
                                                                            @click="toggleCamCaHideWhenZero(element.type, row.renderKey)"
                                                                        />
                                                                        <Button
                                                                            label="Xóa dòng"
                                                                            severity="danger"
                                                            ...
                                                            >
                                                                ...
                                                                <Button
                                                                    ...
                                                                    size="small"
                                                                    variant="outlined"
                                                                    @click="addCamCaChildRow(element.type, row.renderKey, 'child-value')"
                                                                />
                                                                <Button
                                                                    label="+ Dòng CT"
                                                                    size="small"
