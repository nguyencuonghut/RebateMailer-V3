...
class MailPageTest extends TestCase
{
    ...
    public function test_mail_page_exposes_campaign_progress_payload(): void
    {
        ...
        $this->actingAs($user)
            ...
            ->assertInertia(fn (Assert $page) => $page
                ...
                ->where('selectedCampaign.latestPdfExport.statusDetail', 'Yêu cầu đã được ghi nhận và đang chờ worker lấy job.')
    ...
    public function test_mail_page_allows_retrying_pdf_export_after_failed_attempt(): void
    {
        $user = User::query()->where('email', 'user@rebatemailer.test')->firstOrFail();
        ...
        $campaign = MailCampaign::query()->create([
            'name' => 'Chiến dịch tháng 9',
            'import_batch_id' => $batch->id,
            'mail_template_canvas_id' => $canvas->id,
            'notes' => 'Retry export PDF',
        ...
        \DB::table('mail_campaign_exports')->insert([
            ...
            'total_recipients' => 1,
            'exported_recipients' => 0,
            'error_message' => 'Recipient 90300 thiếu snapshot cần thiết để export PDF.',
        ...
        $this->actingAs($user)
            ...
            ->assertInertia(fn (Assert $page) => $page
                ...
                ->where('selectedCampaign.latestPdfExport.statusDetail', 'Quá trình tạo PDF đã thất bại.')
